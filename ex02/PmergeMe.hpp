...
T PmergeMe::sortPairs(T& cont, size_t compFactor, size_t pairNum)
{
	...
	if (isOdd)
	{
		...
		if (DEBUG) printContainer(remainder, "Remainder", 1, 1);
		return remainder;
	}
...
template <typename T>
void PmergeMe::initMainPend(T& cont, T& main, T& pend, size_t compFactor, size_t pairNum, size_t level)
{
	if (pairNum > 1)
	{
	...
	if (DEBUG)
	{
		std::cout << BOLD YELLOW << "\nLevel: " << BCYAN << level << RESET << BOLD YELLOW << ", compFactor: " << BCYAN << compFactor << RESET << BOLD YELLOW << ", pairNum: " << BCYAN << pairNum << "\n" << RESET;
...
template <typename T, typename U>
void PmergeMe::structurePend(U& pendElements, T& pend, size_t compFactor, T& remainder, T& cont)
{
	size_t k = 2;
	if (cont.size() == 3 && remainder.size() == 1)
	{
		...
		pendElements.push_back(std::make_pair(T(1, remainder[0]), k));
	}
	else
	{
...
{
	size_t rightmost = mainElements[0].first.size() - 1;
	size_t upperBound = mainElements.size();
	if (DEBUG)
	{
		std::cout << BOLD BMAGENTA << "Binary search" << RESET << "\n" << MAGENTA << "Defining search range" << RESET << "\n";
		printContainer(pendElements[startIndex].first, "Trying to insert element", 0, 1);
		std::cout << "Currently searching main up to max element: " << BGREEN << "[" << (upperBound ? mainElements[upperBound - 1].second : mainElements[0].second) << "]\n" << RESET;
	}
	for (size_t i = 0; i < mainElements.size(); i++)
	{
		if (mainElements[i].second == pendElements[startIndex].second)
		{
			...
			i == 0 ? upperBound = 0 : upperBound = i;
			break;
		}
	}
	
	if (DEBUG) std::cout << "After adjustment - Searching up to element: " << BGREEN << "[" << (upperBound ? mainElements[upperBound - 1].second : mainElements[0].second) << "]\n" << MAGENTA << "\nFinding an index for insertion\n" << RESET;
	size_t lowerBound = 0;
	size_t mid = 0;
	...
	while (lowerBound < upperBound)
	{
		...
		if (DEBUG)
		{
			std::cout << "In range: " << ORANGE << lowerBound << RESET " - " << ORANGE << upperBound << RESET << " \t|\tStep 1: finding mid index: " ORANGE << mid << RESET << "\t|\t";
...
template <typename U>
size_t PmergeMe::findStartIndex(size_t J, U& pendElements, size_t startIndex, size_t insertionsCount)
{
	if (DEBUG) std::cout << BBLUE << "\nCurrent selected Jacobstahl number: " << BOLD << J << RESET << "\n";
	size_t pendSize = pendElements.size();
	if (pendSize == 0)
		return 0;
	...
	if (DEBUG) std::cout << "Pend index matching " << BBLUE << "J" << RESET << " not found. Starting inserting in reverse from element " BGREEN << "[" << pendElements[pendSize - 1].second << "]" << RESET << "\n";
	return pendSize - 1;
}
...
void PmergeMe::FJMICore(T& cont, U& mainElements, size_t level)
{
	...
	if (DEBUG) printContainer(cont, "Container after pair sorting", 1, 1);
	...
	initMainPend(cont, main, pend, compFactor, pairNum, level);
	...
	structurePend(pendElements, pend, compFactor, remainder, cont);

	if (DEBUG) std::cout << "\n---\n";

	// 4.4. Insert elements from pend into main based on Jacobstahl numbers ordering.
	size_t n = 0, insertionsCount = 0, startIndex = 0;
	while (!pendElements.empty())
	{
		size_t J = (n >= jacobstahlSequence.size()) ? 0 : jacobstahlSequence[n];
		
		if (J != 0)
		{
			size_t blockSize = (n == 0) ? 2 : (jacobstahlSequence[n] - jacobstahlSequence[n - 1]);
			if (insertionsCount == blockSize)
			{
				n++;
				insertionsCount = 0;
				continue;
		...
		startIndex = findStartIndex(J, pendElements, startIndex, insertionsCount);
	...
	// 4.5. Fill the original container with the result of insertion at this level to carry over to the next level.
